...
def getIMG(fns, readDIR, saveDIR):
    ...
    for fn in fns:
        ...
        for url in furl.readlines():
            ...
            try:
                urllib.request.urlretrieve(url, saveDIR + '/' + name + '/%d.jpg' % count)
            except:
                continue
            print
            'Downloading: %s ---- #%d' % (name, count)
...
if __name__ == '__main__':
    # example:
    url_saveDIR = r'./google_image/url'
    img_saveDIR = r'./google_image/images'
    search_items = ['Women striped top forever 21']
    #getIMGurlsGoogle(search_items=search_items, num=1000, bottom=10000, saveDIR=url_saveDIR, items_per_round=10)
    getIMG(['womenstrippedtop.txt'], readDIR=url_saveDIR, saveDIR=img_saveDIR)
