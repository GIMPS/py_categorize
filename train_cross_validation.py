...
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['total']}
train_dataset_len = len(image_datasets['total'])
class_names = image_datasets['total'].classes
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        image_datasets['val'], image_datasets['train'] = random_split(image_datasets['total'], [train_dataset_len // 5,
                                                                                                train_dataset_len - train_dataset_len // 5])
